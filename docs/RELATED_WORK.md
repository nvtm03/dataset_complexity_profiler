...
| **Критерий качества** (*Performance Measure*) | Сохранение ≥ 97% качества линейной модели (точность `RidgeClassifier` для классов, \|Spearman\| косинусной близости для STS) относительно полного 384-мерного вектора. |
...
2. **Landmarking через линейные модели:** концепция *landmarking* (Pfahringer et al., 2000) реализована через быстрые линейные модели (`RidgeClassifier` для классификации и ранговая корреляция `|Spearman|` косинусной близости для STS), определяющие разделимость пространства до запуска тяжелых вычислений.
